...
The system SHALL read the `implicitUsings` string array from `twohash.config.json`. When present, these SHALL **replace** the built-in default global usings entirely. Each entry SHALL be prepended as a `global using <namespace>;` directive before compilation. These usings SHALL NOT appear in the output code.

#### Scenario: Config replaces default implicit usings
- **WHEN** `twohash.config.json` contains `{"implicitUsings": ["System.Text", "System.Text.Json"]}`
...
- **WHEN** `twohash.config.json` contains `{"implicitUsings": ["System", "System.Linq", "System.Text.Json"]}`
...
- **WHEN** `twohash.config.json` contains `{"implicitUsings": []}`
...
- **WHEN** `twohash.config.json` contains `{"framework": "net9.0"}` with no `implicitUsings`
...
- **WHEN** no `twohash.config.json` exists
