...
- **WHEN** `glosharp.config.json` contains `{"langVersion": "12"}` and a code block has no `@langVersion` marker
- **THEN** the code block is compiled with C# 12 language version

#### Scenario: Per-block marker overrides config langVersion
- **WHEN** `glosharp.config.json` contains `{"langVersion": "12"}` and a code block contains `// @langVersion: preview`
...
- **WHEN** `glosharp.config.json` has no `langVersion` property and a code block has no `@langVersion` marker
...
The system SHALL read the `nullable` string property from `glosharp.config.json` and use it as the default nullable context for all code blocks. Per-block `@nullable` markers SHALL override the config value.

#### Scenario: Config sets nullable baseline
- **WHEN** `glosharp.config.json` contains `{"nullable": "disable"}` and a code block has no `@nullable` marker
- **THEN** the code block is compiled with nullable context disabled

#### Scenario: Per-block marker overrides config nullable
- **WHEN** `glosharp.config.json` contains `{"nullable": "disable"}` and a code block contains `// @nullable: enable`
...
- **WHEN** `glosharp.config.json` has no `nullable` property and a code block has no `@nullable` marker
