...
- **WHEN** `glosharp init` is run in a directory without `glosharp.config.json`
- **THEN** a `glosharp.config.json` file is created with default values and the CLI exits with code 0
...
The CLI SHALL refuse to overwrite an existing `glosharp.config.json` unless `--force` is specified.

#### Scenario: Config already exists
- **WHEN** `glosharp init` is run in a directory that already contains `glosharp.config.json`
- **THEN** the CLI exits with non-zero code and writes an error to stderr indicating the file already exists

#### Scenario: Force overwrite
- **WHEN** `glosharp init --force` is run in a directory that already contains `glosharp.config.json`
...
- **WHEN** `glosharp init` creates a config file
