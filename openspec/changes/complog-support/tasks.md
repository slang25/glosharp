...
- [x] 1.1 Add `Basic.CompilerLog.Util` NuGet package reference to `GloSharp.Core.csproj`
- [x] 1.2 Verify the package restores and the project builds cleanly

## 2. ComplogResolver

- [x] 2.1 Create `ComplogResolver` class in `GloSharp.Core` that opens a complog file, selects a compilation (by project name or first C# compilation), and extracts `MetadataReference[]`, `CSharpCompilationOptions`, `CSharpParseOptions`, target framework, and package list
...
- [x] 3.1 Add `ComplogPath` and `ComplogProject` properties to `GloSharpProcessorOptions`
- [x] 3.2 Add complog resolution path in `GloSharpProcessor.ProcessAsync()` that bypasses FrameworkResolver/ProjectAssetsResolver/FileBasedAppResolver when complog is specified
...
- [x] 4.1 Add `Complog` property (string, nullable) to `GloSharpMeta` model
...
- [x] 6.1 Add `complog` and `complogProject` properties to `GloSharpConfig` model in `ConfigLoader`
...
- [x] 7.1 Add `complog` and `complogProject` optional properties to `GloSharpOptions` interface
- [x] 7.2 Add `complog` and `complogProject` optional properties to `GloSharpProcessOptions` interface
...
- [x] 8.2 Write tests for `GloSharpProcessor` with complog: verify references from complog are used, verify meta fields populated, verify mutual exclusivity with project
