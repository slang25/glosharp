...
- **WHEN** `twohash process snippet.cs --complog build.complog` is run
...
- **WHEN** `twohash process snippet.cs` is run and `twohash.config.json` contains `{"complog": "./build.complog"}`
- **THEN** the CLI uses the complog path from config

#### Scenario: Complog file not found
- **WHEN** `twohash process snippet.cs --complog nonexistent.complog` is run
...
- **WHEN** `twohash verify samples/ --complog build.complog` is run
...
- **WHEN** `twohash render snippet.cs --complog build.complog` is run
...
- **WHEN** `twohash process snippet.cs --complog build.complog --complog-project MyLib` is run
- **THEN** the CLI uses the `MyLib` compilation from the complog

#### Scenario: Complog-project without complog
- **WHEN** `twohash process snippet.cs --complog-project MyLib` is run without `--complog`
...
- **WHEN** `twohash process snippet.cs --complog build.complog --project MyProject.csproj` is run
