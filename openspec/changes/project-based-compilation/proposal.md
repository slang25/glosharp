...
- Populate `TwohashMeta.Packages` with resolved package names and versions
- Add `project` option to the Node.js bridge's `TwohashProcessOptions`
...
- **TwoHash.Core**: New `ProjectAssetsResolver` class; `TwohashProcessorOptions` gains `ProjectPath` property; `TwohashProcessor.Process()` merges project references with framework references
- **TwoHash.Cli**: Argument parsing for `--project` and `--no-restore`; optional `dotnet restore` invocation
