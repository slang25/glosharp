...
- [x] 1.1 Add `CacheDir` property to `GloSharpProcessorOptions` in `Models.cs`
- [x] 1.2 Add assembly version helper to compute glosharp version string for cache keys
...
- [x] 2.2 Integrate `CompilationContextCache` into `GloSharpProcessor` reference resolution — wrap FrameworkResolver, ProjectAssetsResolver, and FileBasedAppResolver calls
...
- [x] 3.6 Integrate `ResultCache` into `GloSharpProcessor.ProcessAsync()` — check cache before processing, write on miss
...
- [x] 4.3 Pass `CacheDir` through to `GloSharpProcessorOptions` in both commands
- [x] 4.4 Add CLI integration tests: process with cache dir, verify with cache dir

## 5. Node Bridge

- [x] 5.1 Add `cacheDir?: string` to `GloSharpOptions` and `GloSharpProcessOptions` in `types.ts`
- [x] 5.2 Pass `cacheDir` as `--cache-dir` argument in CLI spawn logic in `glosharp.ts` (instance-level default, per-call override)
