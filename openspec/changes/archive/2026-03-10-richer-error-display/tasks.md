...
- [x] 1.1 Add `EndLine` and `EndCharacter` nullable int properties to `GloSharpError` in `Models.cs`
- [x] 1.2 Add `WarningColor`, `WarningBackground`, `InfoColor`, `InfoBackground` properties to `GloSharpTheme` and populate for both `github-dark` and `github-light` built-in themes
- [x] 1.3 Update diagnostic extraction in `GloSharpProcessor.cs` to extract end line/character from `Diagnostic.Location` when span crosses multiple lines; omit when single-line
...
- [x] 2.2 Add `glosharp-severity-{severity}` CSS class to error underline and message elements
...
- [x] 3.1 Add optional `endLine?: number` and `endCharacter?: number` to `GloSharpError` interface in `types.ts`
...
- [x] 4.2 Update `GloSharpErrorAnnotation` to carry severity and apply severity-specific CSS class and underline color
...
- [x] 5.1 Update `injectErrors` to add `glosharp-severity-{severity}` class to underline and message elements
