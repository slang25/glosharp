...
- **BREAKING**: `TwohashHover.Docs` changes from `string?` to a `TwohashDocComment?` object with typed fields for each tag category
...
- `json-output`: Hover `docs` field changes from `string | null` to a structured `TwohashDocComment | null` object
- `node-bridge`: TypeScript types updated for the new `TwohashDocComment` interface
- `expressive-code-plugin`: Popup rendering displays structured doc sections (params, returns, remarks, etc.)

## Impact

- **C# core** (`TwoHash.Core`): `Models.cs` (new `TwohashDocComment` model), `TwohashProcessor.cs` (`ExtractDocComment` rewrite)
- **JSON output**: `JsonOutput.cs` serialization handles new nested object (existing camelCase + omit-null config should work)
- **Node bridge** (`packages/twohash`): `types.ts` type changes
