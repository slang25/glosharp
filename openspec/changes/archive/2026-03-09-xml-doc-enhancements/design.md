...
Twohash extracts hover information from C# code using Roslyn. The `ExtractDocComment()` method in `TwohashProcessor.cs` calls `symbol.GetDocumentationCommentXml()` which returns the full XML doc comment, but currently only the `<summary>` element is extracted. The result is stored as `TwohashHover.Docs` — a plain `string?`.

The XML returned by Roslyn can contain rich structured documentation: `<param>`, `<returns>`, `<remarks>`, `<example>`, `<exception>`, `<typeparam>`, `<seealso>`, and more. VS Code tooltips display all of these — twohash should too.
...
- Represent docs as a structured object (`TwohashDocComment`) instead of a flat string
...
**Options:**
- (a) **Typed object** — `TwohashDocComment` with named fields: `summary`, `params`, `returns`, `remarks`, `examples`, `exceptions`
...
**Decision:** Add new CSS rules to the existing `buildBaseStyles()` function in `plugin.ts`. Each doc section gets a class: `.twohash-popup-params`, `.twohash-popup-returns`, `.twohash-popup-remarks`, `.twohash-popup-example`, `.twohash-popup-exceptions`. All share the separator border pattern already used by `.twohash-popup-docs`.

## Risks / Trade-offs

- **[Breaking change]** → The `docs` field type changes from `string | null` to `TwohashDocComment | null`. Any consumers reading `hover.docs` as a string will break. Mitigated by: this is pre-1.0 software with no external consumers yet.
