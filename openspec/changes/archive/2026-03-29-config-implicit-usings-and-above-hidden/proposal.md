...
- Add `implicitUsings` string array to `glosharp.config.json` — when specified, these **replace** the built-in default global usings entirely (matching .NET SDK semantics). When omitted, the current defaults apply.
- Add `langVersion` and `nullable` string properties to `glosharp.config.json` — config sets the baseline, per-block `@langVersion`/`@nullable` markers override.
...
### New Capabilities
- `config-implicit-usings`: Configurable implicit usings via `glosharp.config.json` that replace the built-in defaults when specified
...
- `GloSharpConfig` model gains `implicitUsings`, `langVersion`, and `nullable` properties
- `GloSharpProcessorOptions` gains `ImplicitUsings`, `LangVersion`, and `Nullable` properties
- `GloSharpProcessor` uses config usings to replace defaults when specified; uses config `langVersion`/`nullable` as fallback before marker overrides
