...
- **Fix EC plugin styleSettings export**: Stop returning a plain `styleSettings` object from `pluginTwohash()` since the styles are already embedded in `baseStyles`, removing the need for consumers to strip it with a workaround.
...
- **Core C#** (`src/TwoHash.Core/TwohashProcessor.cs`): Modified hover extraction logic in `BuildHoverFromToken`
- **Core C#** (`src/TwoHash.Core/MarkerParser.cs`): New directive parsing, new fields on `MarkerParseResult`
- **Core C#** (`src/TwoHash.Core/TwohashProcessor.cs`): Modified diagnostic extraction to honor block-level suppression
- **EC plugin** (`packages/expressive-code/src/plugin.ts`): Remove or fix `styleSettings` export
- **Node types** (`packages/twohash/src/types.ts`): May need updates if suppression state is surfaced in output
