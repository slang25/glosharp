...
- [x] 3.1 In `TwohashProcessor.ExtractDiagnostics`, apply block-level suppression: if `SuppressAllErrors` is true, suppress all errors; if `SuppressedErrorCodes` has entries, suppress matching codes. Apply before per-line `@errors` matching. Add conflict detection for `@suppressErrors` + `@noErrors`.
...
- [x] 4.1 In `packages/expressive-code/src/plugin.ts`, remove the `styleSettings` property from the plugin object returned by `pluginTwohash()`. Verify that `baseStyles` already contains all theme-aware CSS custom properties.
...
- [x] 5.1 Run the full test suite (`dotnet run --project tests/TwoHash.Tests/`) and ensure all existing tests pass alongside new tests.
