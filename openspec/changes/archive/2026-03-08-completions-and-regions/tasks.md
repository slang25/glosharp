...
- [x] 2.1 Add `TwohashCompletion` and `TwohashCompletionItem` model classes to `Models.cs`
- [x] 2.2 Replace `List<object> Completions` with `List<TwohashCompletion> Completions` in `TwohashResult`
- [x] 2.3 Make `TwohashProcessor.Process()` async (return `Task<TwohashResult>`)
...
- [x] 2.5 Map `CompletionItem` results to `TwohashCompletionItem` (label, kind from tags, detail from properties)
...
- [x] 3.2 Add `RegionName` property to `TwohashProcessorOptions`
- [x] 3.3 Implement region extraction in `TwohashProcessor`: when region specified, mark lines outside the region as hidden (reuse hide/show infrastructure)
...
- [x] 4.4 Pass `RegionName` to `TwohashProcessorOptions`
...
- [x] 5.1 Add `TwohashCompletion` and `TwohashCompletionItem` interfaces to `types.ts`
- [x] 5.2 Update `TwohashResult.completions` type from `object[]` to `TwohashCompletion[]`
- [x] 5.3 Add `region` to `TwohashProcessOptions` interface
...
- [x] 6.2 Implement completion list HAST injection in `root` hook: `<ul class="twohash-completion-list">` with `<li>` items containing label, kind badge, and detail
...
- [x] 7.2 Create `TwohashCompletionAnnotation` class extending `ExpressiveCodeAnnotation` with completion list rendering
