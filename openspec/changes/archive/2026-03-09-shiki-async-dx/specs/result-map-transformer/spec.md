...
The package SHALL export a `transformerTwohashFromMap(resultMap)` function that accepts a `TwohashResultMap` and returns a `ShikiTransformer`.

#### Scenario: Create transformer from result map
- **WHEN** `transformerTwohashFromMap(resultMap)` is called with a populated result map
...
- **THEN** it returns the cleaned code from the corresponding `TwohashResult`
...
- **THEN** the HAST tree contains a `<span class="twohash-hover">` wrapping the token with CSS anchor positioning and a sibling `<div class="twohash-popup">` with formatted display parts
...
- **THEN** the HAST tree contains a `<ul class="twohash-completion-list">` with completion items
...
A single transformer instance returned by `transformerTwohashFromMap` SHALL correctly handle multiple sequential `codeToHtml` calls, matching each to the correct result in the map.
...
The transformer SHALL have `name: 'twohash'` for identification in Shiki's transformer pipeline.

#### Scenario: Transformer name
- **WHEN** the transformer is inspected
- **THEN** `transformer.name` equals `'twohash'`
