...
The .NET core/CLI and the npm packages live in one repo. The .NET solution contains `GloSharp.Core` (class library) and `GloSharp.Cli` (dotnet tool). The npm side uses workspaces for `glosharp` (bridge), `@glosharp/shiki`, and `@glosharp/expressive-code`.
...
The Node.js bridge spawns `glosharp` CLI as a child process and parses JSON from stdout. Alternatives considered: compiling Roslyn to WASM (too complex, Roslyn depends heavily on .NET runtime), native Node.js addon via NativeAOT (fragile cross-platform, complex build).
...
For the initial version, glosharp resolves framework reference assemblies from the installed .NET SDK (`~/.dotnet/packs/Microsoft.NETCore.App.Ref/{version}/ref/{tfm}/`). No NuGet packages, no .csproj loading.
