...
- [x] 1.1 Create `ProjectAssetsResolver` class in GloSharp.Core that accepts a project path (file or directory) and locates `obj/project.assets.json`
...
- [x] 1.6 Extract package name/version metadata for `GloSharpMeta.Packages`
...
- [x] 2.1 Add `ProjectPath` property to `GloSharpProcessorOptions`
- [x] 2.2 Update `GloSharpProcessor.Process()` to merge project references with framework references when `ProjectPath` is set
- [x] 2.3 Populate `GloSharpMeta.Packages` from resolved project metadata
...
- [x] 4.1 Add `project` and `noRestore` to `GloSharpProcessOptions` type
- [x] 4.2 Pass `--project` and `--no-restore` arguments when spawning CLI
- [x] 4.3 Write unit tests for new CLI argument construction

## 5. Shiki Transformer

- [x] 5.1 Add `project` option to `transformerGloSharp()` options type
...
- [x] 6.1 Add `project` option to `pluginGloSharp()` options type
