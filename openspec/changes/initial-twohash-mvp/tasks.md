...
- [x] 1.1 Create .NET solution with `TwoHash.Core` class library and `TwoHash.Cli` console app projects targeting net8.0
- [x] 1.2 Add NuGet references: `Microsoft.CodeAnalysis.CSharp` and `Microsoft.CodeAnalysis.CSharp.Features` to Core project
- [x] 1.3 Configure CLI project as a dotnet tool (pack as `twohash` global tool)
- [x] 1.4 Initialize npm workspace with packages: `twohash`, `@twohash/shiki`, `@twohash/expressive-code`
...
- [x] 4.1 Define `TwohashResult` model classes: `TwohashResult`, `TwohashHover`, `TwohashError`, `TwohashDisplayPart`, `TwohashMeta`
...
- [x] 6.1 Define TypeScript interfaces: `TwohashResult`, `TwohashHover`, `TwohashError`, `TwohashDisplayPart`, `TwohashMeta`, `TwohashOptions`
- [x] 6.2 Implement `createTwohash()` factory with CLI auto-detection and custom executable path
...
- [x] 7.1 Implement `transformerTwohash()` factory returning Shiki transformer object
...
- [x] 7.6 Implement display parts rendering with CSS classes per kind (`twohash-keyword`, `twohash-className`, etc.)
...
- [x] 8.1 Implement `pluginTwohash()` factory returning EC plugin object
- [x] 8.2 Implement `preprocessCode` hook: detect markers, invoke bridge, strip markers from code
- [x] 8.3 Implement `TwohashHoverAnnotation` class extending `ExpressiveCodeAnnotation` with `inlineRange` and `render()`
- [x] 8.4 Implement `TwohashErrorAnnotation` class for error underlines and messages
- [x] 8.5 Implement `annotateCode` hook: create annotations from twohash result
