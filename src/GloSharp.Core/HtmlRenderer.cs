...
{
    /// <summary>
    /// Renders a GloSharpResult as self-contained HTML.
        ...
        HtmlRenderOptions? options = null)
    {
        ...
        if (options.Standalone)
        {
            ...
            sb.AppendLine("<title>glosharp</title>");
            sb.AppendLine("</head>");
            sb.AppendLine("<body>");
        }

        sb.AppendLine($"<div class=\"glosharp-code\" data-theme=\"{Encode(theme.Name)}\">");
        sb.AppendLine("<style>");
        ...
        List<GloSharpError> errors)
    {
        ...
        var lineErrors = new Dictionary<int, GloSharpError>();
        ...
        while (savedTokenIndex < tokens.Count)
        {
            ...
            {
                sb.Append($"<span class=\"glosharp-error-underline glosharp-severity-{error!.Severity}\">");
                sb.Append($"<span{colorAttr}>{Encode(tokenText)}</span>");
                sb.Append("</span>");
            }
    ...
    private static void RenderPopup(StringBuilder sb, GloSharpHover hover, int index, GloSharpTheme theme)
    {
        ...
        if (hover.Docs != null)
        {
            sb.Append("<div class=\"glosharp-popup-docs\">");
            if (hover.Docs.Summary != null)
            {
                sb.Append($"<div class=\"glosharp-popup-summary\">{Encode(hover.Docs.Summary)}</div>");
            }
            if (hover.Docs.Params.Count > 0)
            {
                sb.Append("<div class=\"glosharp-popup-params\">");
                sb.Append("<div class=\"glosharp-popup-section-label\">Parameters</div>");
                foreach (var p in hover.Docs.Params)
                {
                    sb.Append($"<div class=\"glosharp-popup-param\"><span class=\"glosharp-popup-param-name\">{Encode(p.Name)}</span> \u2014 {Encode(p.Text)}</div>");
                }
                sb.Append("</div>");
            }
            if (hover.Docs.Returns != null)
            {
                sb.Append("<div class=\"glosharp-popup-returns\">");
                sb.Append("<div class=\"glosharp-popup-section-label\">Returns</div>");
    ...
    private static void RenderErrorMessage(StringBuilder sb, GloSharpError error)
    {
        sb.Append($"<div class=\"glosharp-error-message glosharp-severity-{error.Severity}\">");
        ...
        if (CsCodeRegex.IsMatch(error.Code))
        {
            ...
            sb.Append($"<a class=\"glosharp-error-code\" href=\"{Encode(codeUrl)}\" target=\"_blank\" rel=\"noopener\">{Encode(error.Code)}</a>");
        }
        else
        {
            sb.Append($"<span class=\"glosharp-error-code\">{Encode(error.Code)}</span>");
        }
    ...
    private static void RenderCompletionList(StringBuilder sb, GloSharpCompletion completion, GloSharpTheme theme)
    {
        sb.Append("<ul class=\"glosharp-completion-list\">");
        foreach (var item in completion.Items)
        {
            sb.Append("<li class=\"glosharp-completion-item\">");
            sb.Append($"<span class=\"glosharp-completion-kind\">{Encode(item.Kind)}</span>");
            sb.Append($"<span class=\"glosharp-completion-label\">{Encode(item.Label)}</span>");
            if (item.Detail != null)
                sb.Append($"<span class=\"glosharp-completion-detail\">{Encode(item.Detail)}</span>");
            sb.Append("</li>");
        }
        sb.AppendLine("</ul>");
    }
