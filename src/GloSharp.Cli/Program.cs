...
static async Task<int> RunProcess(string[] args)
{
    ...
    try
    {
        ...
        var result = await processor.ProcessAsync(source, new GloSharpProcessorOptions
        {
...
static async Task<int> RunRender(string[] args)
{
    ...
    if (theme == null)
    {
        Console.Error.WriteLine($"Error: unknown theme '{themeName}'. Valid themes: {string.Join(", ", GloSharpTheme.BuiltInNames)}");
        return 1;
    }
    ...
    {
        var processor = new GloSharpProcessor();
        var processResult = await processor.ProcessWithContextAsync(source, new GloSharpProcessorOptions
        {
            TargetFramework = framework,
            ProjectPath = project,
            RegionName = region,
...
static int RunInit(string[] args)
{
    ...
    try
    {
        ...
        Console.Error.WriteLine("Created glosharp.config.json");
        Console.Error.WriteLine();
...
static void PrintUsage()
{
    ...
    Console.Error.WriteLine("  --config <path>     Path to glosharp.config.json (default: auto-discover)");
    Console.Error.WriteLine();
