...
public class ConfigLoaderTests
{
    ...
    public void Setup()
    {
        _tempDir = Path.Combine(Path.GetTempPath(), "glosharp-config-test-" + Guid.NewGuid().ToString("N")[..8]);
    ...
    {
        File.WriteAllText(Path.Combine(_tempDir, "glosharp.config.json"), """{"framework": "net9.0"}""");
    ...
    {
        File.WriteAllText(Path.Combine(_tempDir, "glosharp.config.json"), """{"framework": "net8.0"}""");
    ...
    public async Task Discover_NoConfigFound_ReturnsNull()
    {
        ...
        // May or may not be null depending on whether a glosharp.config.json exists
    ...
    public async Task Parse_FullConfig_AllPropertiesParsed()
    {
        ...
        """;
        File.WriteAllText(Path.Combine(_tempDir, "glosharp.config.json"), json);
    ...
    {
        File.WriteAllText(Path.Combine(_tempDir, "glosharp.config.json"), "{}");
    ...
    {
        File.WriteAllText(Path.Combine(_tempDir, "glosharp.config.json"), "{invalid json");
    ...
    {
        var json = """{"project": "./samples/Samples.csproj", "cacheDir": ".glosharp-cache"}""";
        File.WriteAllText(Path.Combine(_tempDir, "glosharp.config.json"), json);
        ...
        await Assert.That(config.CacheDir).IsEqualTo(
            Path.GetFullPath(Path.Combine(_tempDir, ".glosharp-cache")));
    }
    ...
    public async Task WriteDefault_CreatesValidJsonFile()
    {
        ...
        var path = Path.Combine(dir, "glosharp.config.json");
    ...
    public void WriteDefault_RefusesOverwrite()
    {
        ...
        Directory.CreateDirectory(dir);
        File.WriteAllText(Path.Combine(dir, "glosharp.config.json"), "{}");
    ...
    public async Task WriteDefault_ForceOverwrites()
    {
        ...
        Directory.CreateDirectory(dir);
        File.WriteAllText(Path.Combine(dir, "glosharp.config.json"), """{"framework": "old"}""");
