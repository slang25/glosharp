...
public class ResultCacheTests
{
    ...
    public void Setup()
    {
        _cacheDir = Path.Combine(Path.GetTempPath(), "twohash-test-cache-" + Guid.NewGuid().ToString("N")[..8]);
    }
    ...
    private static TwohashResult CreateSampleResult(string code = "var x = 42;") => new()
    {
        Code = code,
        Original = code,
        Hovers = [],
