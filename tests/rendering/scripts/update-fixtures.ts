// Regenerates GloSharpResult JSON fixtures from the curated samples by running
// the GloSharp CLI. With --check, regenerates to memory and diffs against the
// committed fixtures instead of writing, exiting non-zero on divergence.
//
...
async function main() {
  ...
  if (checkMode) {
    ...
    console.log(`All ${fixtures.length} fixtures match the CLI output.`)
    return
  }
